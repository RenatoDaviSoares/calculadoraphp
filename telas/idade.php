...
<body>
    <form method="POST" style="text-align:center;margin:10%;background-color:#aaff34;padding:5%">
        <H1>Calcular Idade em Dias</H1>

        <div class="mb-3">
            <label for="lPrimeiroNumero" class="form-label">Informe a sua idade</label>
            <input type="text" class="form-control" id="primeiroNumero" name="primeiroNumero">
        </div>
        

        <button type="submit" class="btn btn-primary">Calcular idade
            <?php 
                $ano = (float)$_POST['primeiroNumero'];
                //chamo a variavel que representa a classe Funcao
                $resultado = $funcao->calcularIdade($ano);
