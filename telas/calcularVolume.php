...
<body>
    <form method="POST" style="text-align:center;margin:10%;background-color:#aaff34;padding:5%">
        ...
        <div class="mb-3">
            <label for="lSegundoNumero" class="form-label">Informe o Comprimento</label>
            <input type="text" class="form-control" id="segundoNumero" name="segundoNumero">
        </div>
        <div class="mb-3">
            <label for="lTerceiroNumero" class="form-label">Informe a Largura</label>
            <input type="text" class="form-control" id="terceiroNumero" name="terceiroNumero">
        </div>
        

        <button type="submit" class="btn btn-primary">Calcular Volume
            <?php 
                $altura = (float)$_POST['segundoNumero'];
                $comprimento = (float)$_POST['terceiroNumero'];
                $largura = (float)$_POST['primeiroNumero'];
                //chamo a variavel que representa a classe Funcao
                $resultado = $funcao->volume($altura, $comprimento, $largura);
